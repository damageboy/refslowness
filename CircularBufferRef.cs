...
namespace xxx
{
  public class 	CircularBufferRef<T> : IEnumerable<T> where T : struct
  {
    readonly T[] _buffer;
    int _head;

    public CircularBufferRef(int capacity)
    {
      if (capacity < 0)
    ...
    public delegate void NewValue(CircularBufferRef<T> buf, T value);
